...
public class AddBook_Ok_2_IT extends TestNGCitrusTestDesigner {
	...
	static String scriptName = "AddBook_Ok_2_IT";
	...
	public void addBookNewTest() {
		
		final String isbn = "981-citrus:randomNumber(10)";
		// get message here
		extentTest = ExtentTestManager.getTest();
		extentTest.log(LogStatus.INFO, "Sending addBook SOAP message");
		...
		extentTest.log(LogStatus.INFO, "HTML",
				"Book Author - <b>" + message.getPayload().getBook().getAuthor() + "</b>");
		...
		extentTest.log(LogStatus.INFO, "HTML", "Book Registration Date - <b>"
				+ message.getPayload().getBook().getRegistrationDate().getTime() + "</b>");

		// send addBook request
		send(bookStoreClient).payload(createAddBookRequestMessage(isbn), marshaller).header("citrus_soap_action",
				"addBook");

		extentTest.log(LogStatus.INFO, "Sent addBook SOAP Message");
		...
		receive(bookStoreClient).validationCallback(new XmlMarshallingValidationCallback<AddBookResponseMessage>() {
			...
			public void validate(AddBookResponseMessage response, Map<String, Object> headers, TestContext context) {
				if (response.isSuccess()) {
					extentTest.log(LogStatus.INFO, "Received addBook SOAP Message");
				} else {
					extentTest.log(LogStatus.INFO, "Not received addBook SOAP Message");
				}
	...
	public static void runOnceBeforeClass() {
		System.out.println("@BeforeClass - AddBook_Ok_2_IT extent - " + extent);
		// AAFTLogger.writeToLog("@BeforeClass - AddBook_Ok_2_IT extent - " +
		// extent);
